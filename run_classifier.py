...
class QuoraProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            guid = "%s - %s" % (set_type, i)
            if set_type == "test":
                question_a = tokenization.convert_to_unicode(line[1])
                question_b = tokenization.convert_to_unicode(line[2])
                label = "0"
            else:
                if not len(line) == 6:
                    continue
...
class STSBProcessor(DataProcessor):
    ...
    def get_labels(self):
        """See base class."""
        return None  # since everything is between 0-5 ["0", "1"]
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if isinstance(feature.label_id, int):
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
                                drop_remainder, float_label=False):
    ...
    name_to_features = {
        ...
        "label_ids": tf.FixedLenFeature([], tf.int64 if not float_label else tf.float32),
...
def create_modelSTSB(bert_config, is_training, input_ids, input_mask, segment_ids,
...
def model_fn_builderSTSB(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        (total_loss, per_example_loss, logits) = create_modelSTSB(
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        train_input_fn = file_based_input_fn_builder(
            ...
            is_training=True,
            drop_remainder=True, float_label=True if task_name=="stsb" else False)
    ...
    if FLAGS.do_eval:
        ...
        eval_input_fn = file_based_input_fn_builder(
            ...
            float_label = True if task_name == "stsb" else False)
